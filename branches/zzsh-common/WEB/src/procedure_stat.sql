...
begin
  ...
  DECLARE i int;
  DECLARE s tinyint;
	DECLARE s_cursor CURSOR FOR SELECT id, name, code_id, code, parent_id FROM  pc_agency WHERE code_id = 10;
...
--					SELECT i, y, q, m;					
   				...
   				IF  s IS NULL THEN
					...
					IF (c_parent_id > 0 AND y >= 2012) THEN
							IF s < 2 THEN
...
begin
  ...
  DECLARE stat_bmwy_num int(10) unsigned;
	...
	DECLARE s_cursor CURSOR FOR SELECT id, name, code_id, code, parent_id, create_datetime as setup_datetime, p_count, zb_num FROM  pc_agency WHERE code_id = 10;
	DECLARE CONTINUE HANDLER FOR NOT FOUND SET done=1;
...
begin
	...
	DECLARE s_cursor CURSOR FOR SELECT id, name, code_id, code, create_datetime as setup_datetime, parent_id FROM  pc_agency WHERE code_id = 10;
	DECLARE CONTINUE HANDLER FOR NOT FOUND SET done=1;
