...
UPDATE `partycommittee`.`pc_remind_lock` SET `status` = '8' where `agency_id` in (318,310,,311,322) and year = 2013 and quarter = 1 and month = 1 and type_id =8




2013-01-09
 ALTER TABLE `pc_remind_config` CHANGE `start_quarter` `start_quarter` TINYINT( 1 ) NOT NULL DEFAULT '0' ;
 ALTER TABLE `pc_remind_lock` ADD INDEX ( `status` ) ;
 UPDATE `partycommittee`.`pc_remind_config` SET `start_quarter` = '-1' WHERE `pc_remind_config`.`id` =3 LIMIT 1 ;
 UPDATE `partycommittee`.`pc_remind_config` SET `end_quarter` = '1' WHERE `pc_remind_config`.`id` =4 LIMIT 1 ;
 ALTER TABLE `pc_agency` CHANGE `invalid_datetime` `invalid_datetime` INT( 11 ) UNSIGNED NOT NULL DEFAULT '0' ;
 ALTER TABLE `pc_agency_stats` ADD `setup_datetime` INT( 11 ) UNSIGNED NOT NULL DEFAULT '0' AFTER `code` , ADD `invalid_datetime` INT( 11 ) UNSIGNED NOT NULL DEFAULT '0' AFTER `setup_datetime` ;
...
) ENGINE=INNODB  DEFAULT CHARSET=utf8 COMMENT='组织生活统计表' AUTO_INCREMENT=1 
...
ALTER TABLE `pc_zzsh_stat` ADD INDEX ( `code` , `setup_datetime` , `invalid_datetime` , `year` , `month` , `type_id` ) ;
ALTER TABLE `pc_zzsh_stat` ADD INDEX ( `code` , `setup_datetime` , `invalid_datetime` , `year` , `type_id` ) ;
ALTER TABLE `pc_zzsh_stat` ADD INDEX ( `code` , `setup_datetime` , `invalid_datetime` , `year` , `quarter` , `type_id` ) ;
 
 修改my.ini , max_allowed_packet= 64M
