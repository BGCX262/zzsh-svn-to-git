...
//		    ByteArrayOutputStream targetFile = new ByteArrayOutputStream(); 
			...
			for(PcWorkPlanZBStatsVo item : yp_stats) {
				...
				//年度计划
				Label G = new Label(6, y, getRateString(item.getReported_rate().toString()));
				wws.addCell(G);
				
				Label H = new Label(7, y, item.getAgency_goodjob().toString());
				wws.addCell(H);
				
				Label I = new Label(8, y, getRateString(item.getReturn_rate().toString()));
				wws.addCell(I);
				
				Label J = new Label(9, y, item.getDelay_rate().toString());
				wws.addCell(J);
				
				Label K = new Label(10, y, getRateString(item.getEva_rate().toString()) );
				wws.addCell(K);
				
				Label L = new Label(11, y, getRateString(item.getEva_1_rate().toString()) );
				wws.addCell(L);
				
				Label M = new Label(12, y, getRateString(item.getEva_2_rate().toString()) );
				wws.addCell(M);
				
				Label N = new Label(13, y, getRateString(item.getEva_3_rate().toString()) );
				wws.addCell(N);
				
				Label O = new Label(14, y, getRateString(item.getEva_4_rate().toString()) );
				wws.addCell(O);			
			...
			for (int i = 2; i < 4; i++) {
				...
				for(PcWorkPlanZBStatsVo item : q_stats) {					
					xx = x;
					Label G = new Label(xx++, y, getRateString(item.getReported_rate().toString()) );
					wws.addCell(G);
					
					Label H = new Label(xx++, y, item.getAgency_goodjob().toString());
					wws.addCell(H);
					
					Label I = new Label(xx++, y, getRateString(item.getReturn_rate().toString()) );
					wws.addCell(I);
					
					Label J = new Label(xx++, y, getRateString(item.getDelay_rate().toString()) );
					wws.addCell(J);
					
					Label K = new Label(xx++, y, getRateString(item.getEva_rate().toString()) );
					wws.addCell(K);
					
					Label L = new Label(xx++, y, getRateString(item.getEva_1_rate().toString()) );
					wws.addCell(L);
					
					Label M = new Label(xx++, y, getRateString(item.getEva_2_rate().toString()) );
					wws.addCell(M);
					
					Label N = new Label(xx++, y, getRateString(item.getEva_3_rate().toString()) );
					wws.addCell(N);
					
					Label O = new Label(xx++, y, getRateString(item.getEva_4_rate().toString()) );
					wws.addCell(O);			
	...
	public String getMeetingZBStatsById(Integer id, Integer year, Integer startM, Integer endM) {
		...
		try{
			...
			for(PcMeetingZBStatsVo item : zwh_stats) {
				...
				Label H = new Label(7, y, getRateString(item.getReported_rate().toString()));
				wws.addCell(H);
				...
				Label L = new Label(11, y, getRateString(item.getEva_rate().toString()) );
				wws.addCell(L);
				
				Label M = new Label(12, y, getRateString(item.getEva_1_rate().toString()) );
				wws.addCell(M);
				
				Label N = new Label(13, y, getRateString(item.getEva_2_rate().toString()) );
				wws.addCell(N);
				
				Label O = new Label(14, y, getRateString(item.getEva_3_rate().toString()) );
				wws.addCell(O);
				
				Label P = new Label(15, y, getRateString(item.getEva_4_rate().toString()) );
				wws.addCell(P);
			...
			for(PcMeetingZBStatsVo item : dydh_stats) {
				...
				Label H = new Label(xx++, y, getRateString(item.getReported_rate().toString()) );
				wws.addCell(H);
				...
				Label L = new Label(xx++, y, getRateString(item.getEva_rate().toString()) );
				wws.addCell(L);
				
				Label M = new Label(xx++, y, getRateString(item.getEva_1_rate().toString()) );
				wws.addCell(M);
				
				Label N = new Label(xx++, y, getRateString(item.getEva_2_rate().toString()) );
				wws.addCell(N);
				
				Label O = new Label(xx++, y, getRateString(item.getEva_3_rate().toString()) );
				wws.addCell(O);
				
				Label P = new Label(xx++, y, getRateString(item.getEva_4_rate().toString()) );
				wws.addCell(P);
...
//			System.out.print(filename);
	...
	private String getRateString(String rate ) {
		...
		if ( Float.parseFloat(rate)  > 1.0000) {
			rate = "1.0";
		}
